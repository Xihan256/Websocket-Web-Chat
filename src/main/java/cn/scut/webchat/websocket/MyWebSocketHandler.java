    ...
    protected void handleTextMessage(WebSocketSession session, TextMessage message) throws Exception {
        ...
        switch (messageType){
            ...
            case "message":{
                ...
                //todo 如果toName是group的话，就提醒所有人
                if(toName.equals("group")){
                    webSocketManager.notifyAllToReceive(fromName);
                }else{
            ...
            case "group":{
                String otherName = messageMap.get("otherName");
                ArrayList<ChatRecord> chatRecord = rlService.getGroupMsg(otherName);
                ChatRecords records = new ChatRecords("group",chatRecord);
                String json = JSON.toJSONString(records);
                session.sendMessage(new TextMessage(json));
                break;
            }
